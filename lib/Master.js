...
Master.prototype.taskWorker = function(worker, task, args, cb) {
  ...
  if (!is.func(task) && !is.nonEmptyObj(task)) {
    debug('Master.taskAll error, bad task:',task);
    return false;
  }
...
Master.prototype.sendFileToWorker = function(worker, task, cb) {
  ...
  if (!id || !self.getWorker(id)) {
    debug('Master.sendTaskToWorker: bad id received: '+id);
    process.nextTick(function() {
      cb(new Error('Master.sendTaskToWorker: bad id received in worker: '+
                   util.inspect(worker)));
    });
    return;
  }
  ...
  self.serializeFile(worker, task, function(err, taskObj) {
    if (err) {
      debug('sendFileToWorker serializeFile'+util.inspect(err));
      return cb(err);
    }
    ...
    request(options, function (err, res, body) {
      if (err) {
        debug('Master.sendTaskToWorker: request error: '+util.inspect(err));
        return cb(err);
      }
...
Master.prototype.sendFuncToWorker = function(worker, task, args, cb) {
  ...
  var id = worker.id;
  if (!id || !self.getWorker(id)) {
    var str = 'Master.sendFuncToWorker: bad worker received: '+
         util.inspect(worker);
    debug(str);
    process.nextTick(function() { cb(str); });
  }
  ...
  request(options, function (err, res, body) {
    if (err) {
      debug('Master.sendFuncToWorker: request error: '+util.inspect(err));
      return cb(err);
    }

    if (res.statusCode !== 200) {
      debug('Master.sendFuncToWorker unexpected response code: '+res.statusCode);
      return cb('error', res, body);
    }
...
Master.prototype.taskResult = function(req, res, taskResult) {
  ...
  this.emit('workerTaskComplete', taskResult.task, taskResult.task.worker,
            taskResult.result);
};
...
Master.prototype.serializeFile = function(worker, task, cb) {
  var self = this;
  if (!is.obj(task)) {
    debug('serializeFile Received bad object for task: ', task);
    process.nextTick(function() {
      cb(new Error('serializeFile Received bad object for task'+
                   util.inspect(task)));
    });
    return;
  }
  ...
  var type = task.type;
  if (!type || type !== 'file') {
    debug('serializeFile bad task',task);
    process.nextTick(function() {
      cb(new Error('serializeFile bad task: '+util.inspect(task)));
    });
    return;
  }

  if (!is.nonEmptyObj(worker)) {
    debug('serializeFile bad worker object:',worker);
    process.nextTick(function() {
      cb(new Error('serializeFile bad worker object: '+
                   util.inspect(worker)));
    });
    return;
  }
