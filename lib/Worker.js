...
function asyncErr(err, cb) {
  ...
  if (!is.func(cb))
    return debug('asyncErr received bad callback: '+inspect(cb));
...
Worker.prototype.runGithub = function(task, cb) {
  if (!is.nonEmptyObj(task))
    return asyncErr(new Error('runGithub: no task received.'), cb);

  if (!is.str(task.user))
    return asyncErr(new Error('runGithub, task has no user: '+
                    inspect(task)), cb);

  if (!is.str(task.repo))
    return asyncErr(new Error('runGithub, task has no repo: '+
                    inspect(task)), cb);

  if (!is.str(task.dir))
    return asyncErr(new Error('runGithub, task has no dir: '+
                    inspect(task)), cb);

  if (!is.obj(task.cmds))
    return asyncErr(new Error('runGithub, task has no cmds: '+
                    inspect(task)), cb);

  if (!is.str(task.file))
    return asyncErr(new Error('runGithub, task has no file property: '+
                    inspect(task.file)), cb);

  if (!is.array(task.args))
    return asyncErr(new Error('runGithub, task has no args: '+
                    inspect(task)), cb);
